...
spatial information by choosing bigger kernel say for example 9x9(validation accuracy higher -> under-fitting)
...
We can see that 9x9 misses more spatial information. For example, in the last row, 2 was detected as 6 though 2 in the
image doesnt have any loop like feature and also in case of the 1st example, it missed the loop in the 9 possibly due
to large kernel size

To clearly see the difference, it would be interesting to visualize the activations of convolution layers.

* 3x3:
+
we have already seen the intermediate activations for model with 8 filters and 3x3 kernel. If we recall, we saw that
it had fair distribution that covered overall part of 7.
