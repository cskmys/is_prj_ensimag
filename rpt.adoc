...
In the conclusion we mentioned that we were able to see almost similar levels of accuracy between layers with 512 and 128 neurons
...
We quickly ran into under-fitting issues, maybe cause less number of neurons didn't enable it to learn more.
Hence, we retain 512 neurons with 2 hidden layers as in last section as shown in figure below:
...
Under 8 filters we ran into risk of under-fitting and over 8, we risk being stuck at plateau where the accuracy for
the test data will never improve. Hence, we will choose 8.
...
Both models make almost same types of errors. But, 32 filter model gets saturated, while with 8 filters we still have
some room to explore.

It'll also be interesting to visualize the activations at convolution layers for them both.

=== Kernel size
We keep the 8 filtered network configuration from the previous section and experiment here for kernel sizes:
3x3, 5x5, 7x7 and 9x9
...
Our image is 28x28, hence, the spatial information is quite localized, it's unlikely that we obtain more helpful
spatial information by choosing bigger kernel say for example 9x9
...
* 9x9:
+
include::./op/Adamaxcategorical_crossentropyrelul0.002e6b64mConv2D8f7x7MaxPooling2D2x2Conv2D8f7x7MaxPooling2D2x2FlattenDense512reluDense512reluDense10softmax/list.adoc[tags=result]

3x3 kernel performs better. By examining the incorrectly classified samples:
...
* 9x9:
+
include::./op/Adamaxcategorical_crossentropyrelul0.002e6b64mConv2D8f7x7MaxPooling2D2x2Conv2D8f7x7MaxPooling2D2x2FlattenDense512reluDense512reluDense10softmax/list.adoc[tags=incorrect]

We can see that 9x9 misses more spatial information. For example, in the last sample, 4 was detected as 1
(maybe because it missed the 2nd line) and also in case of the 2nd example in the sample, 6 was classified as 0
(again same reason as before).

To clearly see the difference, it would be interesting to visualize the activations of convolution layers.

=== Pooling Types:
We consider 2 types: AveragePooling and Maximum pooling. Since our image is greyscale, and the histogram is not
uniformly spread, we expect Maximum Pooling to do better than Average Pooling.
